...
+-------------------------------------------------------------------------+
|                           Driver Device                                 |
|              POST /location  { driver_id, lat, lng, speed }             |
+----------------------------------+--------------------------------------+
                                   |
                         +---------V---------+
                         |   Producer API    |  :8080
                         |   (Go HTTP)       |
                         +---------+---------+
                                   | kafka.Message{ Key: driver_id }
                         +---------V---------+
                         |      Kafka        |  topic: gps.updates
                         |  (gps.updates)    |  partitioned by driver_id
                         +---------+---------+
                                   | ReadMessage (GroupID: cassandra-writer)
                         +---------V---------+
                         |     Consumer      |  (Go, horizontally scalable)
                         +--+----------+-----+
                            |          |
              +-------------V--+   +---V------------------+
              |   Cassandra    |   |        Redis           |
              | driver_locations|   | SET driver:latest:{id} |  <- latest position
              | TTL: 24h       |   | PUB driver:updates:{id} |  <- live push
              | full history   |   +-----------+------------+
              +----------------+               | pub/sub
                                    +----------V----------+
                                    |  WebSocket Handler  |
